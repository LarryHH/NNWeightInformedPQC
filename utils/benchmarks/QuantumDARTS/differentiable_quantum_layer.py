...
# differentiable_quantum_layer.py

def _marginal_probs_from_statevector(sv: Statevector, n_qubits: int, n_classes: int):
    """
    Map full statevector probabilities to C class-probabilities by *mod-C*
    folding of the lowest k bits (k = ceil(log2 C)). This matches QuantumNN's
    interpret: interpret(x) = x % C
    """
    probs = torch.from_numpy(np.abs(sv.data)**2).float()
    if n_classes <= 1:
        out = torch.ones(1, dtype=probs.dtype, device=probs.device)
        return out / (out.sum() + 1e-12)
    k = int(math.ceil(math.log2(n_classes)))
    mask = (1 << k) - 1
    # Basis index of each amplitude (0..2^n - 1)
    idx = torch.arange(probs.numel(), device=probs.device)
    # Take the lowest k bits and fold them into C bins by modulo C
    lowk = (idx & mask)              # 0 .. 2^k-1
    cls  = lowk % n_classes          # 0 .. C-1
    out = torch.zeros(n_classes, dtype=probs.dtype, device=probs.device)
    out.scatter_add_(0, cls, probs)
    return out / (out.sum() + 1e-12)
...
class DifferentiableQuantumCircuit(torch.autograd.Function):
    ...
    def _probs_from_params(x, gate_select_one_hot, rot_params, n_qubits, n_layers, gate_pool, n_classes):
        ...
        out = torch.zeros(batch_size, n_classes, device=x.device, dtype=out_dtype)
        ...
        for i in range(batch_size):
            ...
            probs = _marginal_probs_from_statevector(sv, n_qubits, n_classes)  # (C,)
            out[i] = probs.to(device=x.device, dtype=out_dtype)
        return out
    ...
    def _build_full_circuit(x_i, gate_indices, rot_params, n_qubits, n_layers, gate_pool, fm_reps: int = 2):
        ...
        # USE THE SAME FEATURE MAP AS QuantumNN
        fm = ZZFeatureMap(n_qubits, reps=fm_reps)
        fm_params = list(fm.parameters)
        qc.compose(fm, inplace=True)

        # sampled architecture with numeric params
        param_counter = 0
        for l in range(n_layers):
            for q in range(n_qubits):
        ...
        # bind feature-map params to x (ParameterVector reused across layers → len == n_qubits)
