...
import numpy as np
from vqc_helpers import dictionary_of_actions, dict_of_actions_revert_q

# Define the Double Deep Q-Network (DDQN) class
class DDQN(object):
    """Implementation of a Double Deep Q-Network (DDQN) agent."""
    def __init__(self, conf, action_size, state_size, device):
        """
        Initializes the Double Deep Q-Network agent.
    ...
    def replay(self, batch_size):
        ...
        - Implements Double Q-learning:
            - Uses policy_net to select the action.
            - Uses target_net to compute the Q-value of the action.
        """
        ...
        """ 
        Double DQN Logic:
            - The policy network selects the next action (max action).
            - The target network evaluates the Q-value of that action.
        """
        # Use policy network to select actions and target network to evaluate them
        next_state_actions = self.policy_net.forward(next_state_batch).max(1)[1].detach()
        next_state_values = self.target_net.forward(next_state_batch).gather(1, next_state_actions.unsqueeze(1)).squeeze(1)
